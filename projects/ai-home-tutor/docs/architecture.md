...
- 会話モードでの正誤判定は AI の応答に含まれる判定を用い、記録のキー（どの単語か）はサーバー側で固定する
...
- 保護者が入力した性格メモを AI が毎回参照する。AI による性格の自動推定は行わず、保護者主導の方式にしている

## 6. データ保護と運用

- サーバーは家庭内の PC でローカルに起動して使い、インターネット上には公開していない。保護者画面の Basic 認証は、家庭内で子ども用画面と保護者用画面を切り分けるためのもの
